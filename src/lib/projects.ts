...
export function parseContentfulProject(projectWork?: ProjectWork): Project | null {
    if (!projectWork) {
        return null
    }
    ...
    return {
        ...
        heroBlock2: projectWork.fields.heroBlock2?.map(parseContentfulContentImage) || [],
...
export async function fetchAllProjects(): Promise<Project[]> {
    ...
    return projects.items.map((project) => parseContentfulProject(project) as Project)
}
...
export async function fetchProject({ slug }: FetchProjectOptions): Promise<Project | null> {
