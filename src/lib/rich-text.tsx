      ...
      const handleScroll = () => {
        if (window.scrollY > 0) {
          toast({
            title: "📱 Heads up!",
            description:
              "Figma embeds can be hard to navigate on mobile. Consider viewing my project pages on a desktop browser for the best experience.",
          });
  ...
  const options = {
    renderNode: {
      [BLOCKS.HEADING_1]: (node: Block | Inline, children: React.ReactNode) => (
        <h1 className="text-2xl md:text-3xl font-semibold mb-4 mx-8">
          {children}
        </h1>
      ),
      [BLOCKS.HEADING_2]: (node: Block | Inline, children: React.ReactNode) => (
        <h2 className="text-xl md:text-2xl font-semibold mx-8">{children}</h2>
      ),
      [BLOCKS.HEADING_3]: (node: Block | Inline, children: React.ReactNode) => (
        <h3 className="text-xl md:text-2xl font-medium mx-8">{children}</h3>
      ),
      ...
      [BLOCKS.PARAGRAPH]: (node: Block | Inline, children: React.ReactNode) => (
        <p className="text-lg leading-relaxed mb-8 mx-8">{children}</p>
      ),
      [BLOCKS.OL_LIST]: (node: Block | Inline, children: React.ReactNode) => (
        <ol className="md:list-outside md:list-decimal md:ml-12">{children}</ol>
      ),
      [BLOCKS.UL_LIST]: (node: Block | Inline, children: React.ReactNode) => (
        <ol className="md:list-outside md:list-disc md:ml-12">{children}</ol>
      ),
      ...
      [BLOCKS.EMBEDDED_ENTRY]: (node: Block | Inline) => {
        ...
        switch (contentType) {
          ...
          case "openGraphEmbed":
            ...
            const openGraphImage = parseContentfulContentImage(
      ...
      [INLINES.HYPERLINK]: (
        ...
        <a
          href={node.data.uri}
          className="text-foreground underline decoration-primary decoration-2 underline-offset-4 hover:decoration-primary/80 rounded-md focus-visible:outline-none focus-visible:ring focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background"
        >
          {children}
        </a>
      ),
    ...
    renderMark: {
      ...
      [MARKS.CODE]: (text: React.ReactNode) => (
        <code className="bg-gray-100 dark:bg-card/90 py-1 px-2 text-base font-semibold dark:font-regular rounded-md text-indigo-600 dark:text-teal-400">
          {text}
        </code>
      ),
    },
